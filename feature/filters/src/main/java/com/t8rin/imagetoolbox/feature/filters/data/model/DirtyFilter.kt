...
@FilterInject
internal class DirtyFilter(
    override val value: GmicFilterParams = GmicFilterParams(
        values = listOf("30", "true", "All", "None")
    )
) : GMICFilterTransformation(), Filter.Dirty {

    override val cacheKey: String
        get() = value.hashCode().toString()

    override fun createFilter(): GmicFilter = Dirty(
        amplitude = value[0].toFloat(),
        monochrome = value[1].toBoolean(),
        channel = GmicChannel.valueOf(value[2]),
        valueAction = GmicValueAction.valueOf(value[3])
    )
