...
import com.jhlabs.LensBlurFilter
...
@FilterInject
internal class LensBlurFilter(
    override val value: Quad<Float, Float, Float, Float> = 10f to 5f qto (2f to 255f)
) : JhFilterTransformation(), Filter.LensBlur {

    override val cacheKey: String
        get() = value.hashCode().toString()

    override fun createFilter(): JhFilter = LensBlurFilter().apply {
        radius = value.first
        sides = value.second.roundToInt()
        bloom = value.third
        bloomThreshold = value.fourth
    }
