...
@FilterInject
internal class MakeSeamlessFilter(
    override val value: GmicFilterParams = GmicFilterParams(
        values = listOf("0")
    )
) : GMICFilterTransformation(), Filter.MakeSeamless {

    override val cacheKey: String
        get() = value.hashCode().toString()

    override fun createFilter(): GmicFilter = MakeSeamless(
        equalizeLight = value[0].toFloat()
    )
