...
@FilterInject
internal class MightyDetailsFilter(
    override val value: GmicFilterParams = GmicFilterParams(
        values = listOf("25", "1", "25", "1", "Luminance")
    )
) : GMICFilterTransformation(), Filter.MightyDetails {

    override val cacheKey: String
        get() = value.hashCode().toString()

    override fun createFilter(): GmicFilter = MightyDetails(
        amplitude = value[0].toFloat(),
        amount = value[1].toFloat(),
        scale = value[2].toFloat(),
        smoothness = value[3].toInt(),
