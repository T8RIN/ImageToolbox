...
package com.t8rin.imagetoolbox.feature.pdf_tools.presentation.resize_pages.screenLogic
...
class ResizePdfPagesToolComponent @AssistedInject internal constructor(
    ...
    val uri by _uri
    private val _params = mutableStateOf(PdfResizeParams())
    val params by _params

    init {
        checkPdf(uri = initialUri, onDecrypted = { _uri.value = it })
    }

    override fun getKey(): Pair<String, Uri?> = "resized_pages" to uri

    fun setUri(uri: Uri?) {
        if (uri == null) registerChangesCleared() else registerChanges()
        _uri.update { uri }
        checkPdf(uri = uri, onDecrypted = { _uri.value = it })
    }

    fun updateParams(params: PdfResizeParams) {
        _params.value = params
        registerChanges()
    }

    private suspend fun process(): Uri = pdfManager.resizePdfPages(
        uri = uri.toString(),
        params = params
    ).toUri()
    ...
    override fun saveTo(uri: Uri) {
        doSaving {
            fileController.transferBytes(process().toString(), uri.toString())
