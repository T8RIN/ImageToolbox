...
@NavigationNode(screen = Screen.PdfTools.Metadata::class, mode = NavigationMode.REPLACE)
class MetadataPdfToolComponent @AssistedInject internal constructor(
    ...
    fun setUri(uri: Uri?) {
        ...
        _uri.update { uri }
        checkSelectedPdf(uri)
    }

    private fun checkSelectedPdf(uri: Uri?) {
        checkPdf(
            uri = uri,
