...
interface RMBGNewestLoader {
    val isDownloaded: Boolean
    fun download(): Flow<RemoteResourcesDownloadProgress>

    companion object Companion : RMBGNewestLoader by RMBGNewestLoaderImpl
}

private object RMBGNewestLoaderImpl : RMBGNewestLoader {
    private val _isDownloaded: MutableState<Boolean> =
        mutableStateOf(GenericBackgroundRemover.isDownloaded.value)
    override val isDownloaded: Boolean by _isDownloaded

    init {
        GenericBackgroundRemover.isDownloaded.onEach {
            _isDownloaded.value = it
        }.launchIn(CoroutineScope(Dispatchers.IO))
    }

    override fun download(): Flow<RemoteResourcesDownloadProgress> =
        GenericBackgroundRemover.startDownload().map {
