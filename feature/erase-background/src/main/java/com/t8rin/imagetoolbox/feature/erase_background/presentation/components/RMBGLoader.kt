...
interface RMBGLoader {
    val isDownloaded: Boolean
    fun download(): Flow<RemoteResourcesDownloadProgress>

    companion object Companion : RMBGLoader by RMBGLoaderImpl
}

private object RMBGLoaderImpl : RMBGLoader {
    private val _isDownloaded: MutableState<Boolean> =
        mutableStateOf(RMBGBackgroundRemover.isDownloaded.value)
    override val isDownloaded: Boolean by _isDownloaded

    init {
        RMBGBackgroundRemover.isDownloaded.onEach {
            _isDownloaded.value = it
        }.launchIn(CoroutineScope(Dispatchers.IO))
    }

    override fun download(): Flow<RemoteResourcesDownloadProgress> =
        RMBGBackgroundRemover.startDownload().map {
