...
data object ImageTracer {
            ...
            if (cnt > 0) {
                for (k in workingPalette.indices) {
                    ...
                    if (ratio < minRatio && cnt < cycles - 1) {
                        workingPalette[k][0] = randomPaletteByte(random)
                        workingPalette[k][1] = randomPaletteByte(random)
                        workingPalette[k][2] = randomPaletteByte(random)
                        workingPalette[k][3] = randomPaletteByte(random)
                    }
                }
            }
    ...
    private fun createPalette(colorNumber: Int, random: Random): Array<ByteArray> {
        ...
        if (colorNumber < 8) {
            ...
            for (randomCount in colorCount until colorNumber) {
                palette[randomCount][0] = randomPaletteByte(random)
                palette[randomCount][1] = randomPaletteByte(random)
                palette[randomCount][2] = randomPaletteByte(random)
                palette[randomCount][3] = randomPaletteByte(random)
            }
        }
    ...
    fun samplepalette(
        ...
        for (i in 0 until numberofcolors) {
            val idx = random.nextInt(imgd.data.size / 4) * 4
    ...
    private fun Array<ByteArray>.copyPalette(): Array<ByteArray> =
